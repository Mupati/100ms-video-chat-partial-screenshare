...
const cachedToken = sessionStorage.getItem("100ms_call_token");
...
const joinHmsRoom = async () => {
  try {
    ...
    let authToken;
    if (!cachedToken) {
      authToken = await fetchToken(formData.name, formData.room);
      sessionStorage.setItem("100ms_call_token", authToken);
    }
