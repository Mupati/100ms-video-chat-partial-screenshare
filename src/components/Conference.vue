...
import { toggleGrayScalePlugin } from "../plugins/grayscaleStream";
...
onUnmounted(() => {
  if (allPeers.value.length) hmsActions.leave();
});

const leaveMeeting = () => {
  sessionStorage.removeItem("100ms_call_token");
  hmsActions.leave();
};
...
const onPeerScreenshare = (res: any) => {
  if (res?.id) {
    hmsActions.attachVideo(res.id as HMSTrackID, screenshareRef.value);
  }
};
...
const renderPeers = (peers: HMSPeer[]) => {
  ...
  peers.forEach((peer: HMSPeer) => {
    if (videoRefs[peer.id]) {
      ...
      hmsStore.subscribe(onPeerScreenshare, selectScreenShareByPeerID(peer.id));
...
<template>
  <main class="mx-10 min-h-[80vh]">
    ...
    <div>
      <h3 class="text-center text-white font-bold text-2xl mt-10 mb-5">
        Plugin Controls
      </h3>
      <div
